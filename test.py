...
ProductsLineFile=pd.read_excel("e:\Dropbox\Python\PriceList\Productsline.xls")
...
def valueProduct(productname,column):
    """
    获取单个产品型号在ProductLine中对应列中的值
    """
    valueName="none"
    for index in ProductsLineFile.index:
        method=str(ProductsLineFile.ix[index]['method']).split(',')
        for methodDetail in method:
            if re.match("^"+str(methodDetail)+".*$", productname):
                valueName=ProductsLineFile.ix[index][column]
    return valueName
    
def getSeriesProducts(column):
    """
    ...
    for line in pricelist.index:
        name = line
        productline=valueProduct(line,column)
...
def getDataFramePrice():
    ...
    获取价格信息表
    """
    price=annexSeriesPrice(method)
    abr=getSeriesProducts('productsLine')
    basicDiscount=getSeriesProducts('basicDiscount')
    discount=getSeriesProducts('discount')
    priceFrame=pd.concat([abr, price, basicDiscount, discount], join='inner', axis=1)
    return priceFrame
